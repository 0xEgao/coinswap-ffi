...
export const liveTestsEnabled = process.env.OPENSWAP_LIVE_TESTS === '1'

const RPC_AUTH_ARGS = ['-regtest', '-rpcport=18442', '-rpcuser=user', '-rpcpassword=password']

function dockerContainers(): string[] {
  const envContainer = process.env.OPENSWAP_DOCKER_CONTAINER?.trim()
  const values = [envContainer, 'openswap-bitcoind', 'openswap-ffi-bitcoind'].filter(
...
export function cleanupWallet(walletName: string) {
  const walletsDir = path.join(os.homedir(), '.openswap', 'taker', 'wallets')
