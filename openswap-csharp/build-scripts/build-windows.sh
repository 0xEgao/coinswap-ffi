#!/usr/bin/env bash
#
# Build the openswap_ffi native library for Windows and stage it under
# Openswap/runtimes/<rid>/native. Intended to run on a Windows runner (Git Bash /
...
LIB_NAME="openswap_ffi.dll"
DEST="$CSHARP_DIR/Openswap/runtimes/$RID/native"

(
  cd "$FFI_COMMONS_DIR"
  rustup target add "$TARGET"
  cargo build --package openswap-ffi --profile "$PROFILE" --target "$TARGET"
)
