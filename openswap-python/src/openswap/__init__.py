"""Openswap Python bindings package."""
...
def _load_bindings():
    try:
        return import_module(".openswap", __name__)
    except Exception as exc:  # pragma: no cover - simple import shim
        raise ImportError(
            "Openswap bindings are not generated. Run ffi-commons/create_bindings.sh "
            "to generate the Python bindings (openswap.py and native libraries) and "
