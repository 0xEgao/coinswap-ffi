...
mkdir -p ../openswap-kotlin/lib/src/main/jniLibs/$RESOURCE_DIR/
echo "🔧 Generating Kotlin bindings..."
cargo run --bin uniffi-bindgen generate \
    --library ./target/$COMPILATION_TARGET/debug/$LIB_NAME \
    --language kotlin \
    --out-dir ../openswap-kotlin/lib/src/main/kotlin/ \
    --no-format

# Copy the native library to jniLibs
echo "📋 Copying native library..."
cp ./target/$COMPILATION_TARGET/debug/$LIB_NAME \
   ../openswap-kotlin/lib/src/main/jniLibs/$RESOURCE_DIR/
cp ./target/$COMPILATION_TARGET/debug/libopenswap_ffi.d \
   ../openswap-kotlin/lib/src/main/jniLibs/$RESOURCE_DIR/
