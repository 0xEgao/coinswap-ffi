/**
 * iOS Example for Openswap Swift bindings
 * 
 * Demonstrates how to integrate openswap into an iOS application
    ...
    func performSwap(amount: UInt64) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            do {
                ...
                let report = try self?.taker?.doOpenswap(swapParams: params)
