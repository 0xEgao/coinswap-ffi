...
# build openswap-ffi rust lib for apple targets
MACOSX_DEPLOYMENT_TARGET="${MACOS_DEPLOYMENT_TARGET}" cargo build --package openswap-ffi --profile release-smaller --target x86_64-apple-darwin
MACOSX_DEPLOYMENT_TARGET="${MACOS_DEPLOYMENT_TARGET}" cargo build --package openswap-ffi --profile release-smaller --target aarch64-apple-darwin
IPHONEOS_DEPLOYMENT_TARGET=14.0 cargo build --package openswap-ffi --profile release-smaller --target x86_64-apple-ios
IPHONEOS_DEPLOYMENT_TARGET=14.0 cargo build --package openswap-ffi --profile release-smaller --target aarch64-apple-ios
IPHONEOS_DEPLOYMENT_TARGET=14.0 cargo build --package openswap-ffi --profile release-smaller --target aarch64-apple-ios-sim

# build openswap-ffi Swift bindings and put in openswap-swift Sources
...
# combine openswap-ffi static libs for aarch64 and x86_64 targets via lipo tool
...
# create new xcframework directory from openswap-ffi static libs and headers
